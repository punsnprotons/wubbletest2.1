...
let onClientConnect = (ws) => { 
   console.log(`client connected with session Id - ${ws.sessionId}`)
}
let onClientClose = (ws) => { 
  console.log(`client disconnected with session Id - ${ws.sessionId}`)
}
      ...
      if (wss) {
        wss.on('connection', (ws, req) => {
          ...
          if(params.query.sessionId) {
            ws.sessionId = params.query.sessionId
          }
